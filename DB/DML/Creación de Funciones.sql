...
CREATE function sf_get_user_id(@conn_guid UNIQUEIDENTIFIER)
RETURNS varchar(255)
AS
BEGIN
...
SET @response = (select user_id from _sap_conn where conn_guid = @conn_guid) 

RETURN @response;

/********************************* TEST UNITARIO*********************************
DECLARE @ret nvarchar(255);
EXEC @ret = sf_get_user_id @conn_guid = '7ADCE95B-8496-49C5-A7EF-3A9362D15EE6' ;
select @ret;

SELECT * from _sap_conn;

 si la length guid no es convencional devuelve NULL;;;;;;;
*********************************************************************************/
