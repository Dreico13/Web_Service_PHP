...
BEGIN
    ...
    BEGIN
		...
		if(@ret = 1)
		BEGIN
		EXEC sp_sap_utils_XMlresponse @ret,@message = 'este usuario ya tiene conexión'
		END
		ELSE
		BEGIN
...
CREATE or alter PROCEDURE sp_cart_XMLresponse
    @user_id nvarchar(255)
AS
BEGIN
   SELECT
       CONVERT(int, 0) AS [error],
       (
           SELECT
           ...
           FROM _sap_cart cart 
           JOIN _sap_products product ON cart.product_id = product.id
...
BEGIN
    ...
    IF(@ret = 0)
        BEGIN
            EXEC @user_id = sf_get_user_id @conn_guid
            IF EXISTS (SELECT 1 FROM _sap_cart WHERE product_id = @product_id AND user_id = @user_id)
            BEGIN
                UPDATE _sap_cart
            ...
            ELSE
                BEGIN
                    INSERT INTO _sap_cart (product_id, user_id, quantity)
                    VALUES (@product_id, @user_id, @quantity);
                END;
        EXEC sp_sap_utils_XMlresponse @ret,@message = 'producto añadido'
        END;
    ELSE
        BEGIN
            EXEC sp_sap_utils_XMlresponse @ret,@message = 'sesion inexistente';
...
-- EXEC sp_add_to_cart @product_id = 1, @conn_guid = "7D021356-1327-47EB-9508-23108F1280C8", @quantity = 1
...
BEGIN
    ...
    IF(@ret = 0)
        BEGIN
            EXEC @user_id = sf_get_user_id @conn_guid
            EXEC sp_cart_XMLresponse @user_id;
        END;
    ELSE
        BEGIN
            EXEC sp_sap_utils_XMlresponse @ret,@message = 'sesion inexistente';
...
/********************************************************************************************************************/
/* exec sp_get_cart "f72f7679-fcec-434e-bc5d-787a8dfc7fcf"
    select * from _sap_cart;
*/
/***********************************************************************************/

CREATE or alter PROCEDURE sp_drop_from_cart
    @product_id INT,
    @conn_guid UNIQUEIDENTIFIER
AS
BEGIN
    ...
    BEGIN
        EXEC @user_id = sf_get_user_id @conn_guid;
        DELETE FROM _sap_cart WHERE user_id = @user_id AND product_id = @product_id;  
        EXEC sp_sap_utils_XMlresponse @ret, @message = 'producto eliminado';
    END
    ELSE
    BEGIN
        EXEC sp_sap_utils_XMlresponse @ret, @message = 'sesion inexistente';
...
/********************************************************************************************************************/
/* exec sp_drop_from_cart 4,"f72f7679-fcec-434e-bc5d-787a8dfc7fcf"
    select * from _sap_cart;
*/
/***********************************************************************************/